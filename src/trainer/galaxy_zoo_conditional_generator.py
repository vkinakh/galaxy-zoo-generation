...
from src.models import GlobalDiscriminator
from src.models import NLayerDiscriminator
...
class GalaxyZooConditionalGeneratorTrainer(GeneratorTrainer):

    """Trainer for conditional generation of Galaxy Zoo dataset"""
    ...
    def generate_samples(self, samples_folder: PathOrStr, n_samples: int, rows: int = 4):
        loader = self._get_dl()

        log_sample = next(loader)[1][:n_samples]
        log_sample = log_sample.to(self._device)

        with torch.no_grad():
            utils.save_image(
                self._g_ema(log_sample),
                samples_folder / 'samples.png',
                nrow=rows,
                normalize=True,
                value_range=(-1, 1),
            )
